...
def configure_firewall_rule():
    ...
    return oc_fw_module.set_up_rule_on_interfaces(firewall_data)


@app.route('/delete-firewall-rule', methods=['DELETE'])
def delete_firewall_rule():
    global oc_fw_module
    return oc_fw_module.reset_firewall(request.data)
...
def update_firewall_rule():
    ...
    return oc_fw_module.set_up_rule_on_interfaces(request.data)
...
def configure_conntrack_sync():
    ...
    try:
        ...
        # error handler.
        return send_error_respponse(OCException(err[0], status_code=err[1],
                                    payload=err[2]))
    else:
...
def send_error_respponse(error):
...
def add_rule():
    # Updating sshd conf to listen on mgmt ip address
...
def main():
    ...
    try:
        ...
        logger.error("Error in adding management pbr routes %s" % str(ex))
    ip_addr = get_interface_to_bind()
    #oc_fw_module.run_sshd_on_mgmt_ip(ip_addr)
