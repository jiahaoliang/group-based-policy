...
from gbpservice.neutron.nsf.configurator.modules.vpn import RestApi
from gbpservice.neutron.nsf.configurator.modules.vpn import VPNSvcValidator
...
class VpnaasIpsecDriver(object):
    """
...
class VpnaasSslDriver(object):
    ...
    @property
    def service_type(self):
        return "%s-%s" % (const.VYOS, const.SERVICE_TYPE_OPENVPN)
    ...
    def _init_state(self, context, conn):
        LOG.emit("info", "SSL: Configured successfully- %s " % conn['id'])
        self.agent.update_conn_status(
            context,
            const.SERVICE_TYPE_OPENVPN,
            conn,
    ...
    def _sslvpn_get_tenant_conns(self, context, conn, on_delete=False):
        ...
        tenant_conns = self.agent.get_ssl_vpn_conns(
            context, filters=filters)
        if not tenant_conns:
            if not on_delete:
                # Something went wrong - atleast the current
                # connection should be there
                msg = _("SSL: No tenant conns for filters (%s)"
                        % (str(filters)))
                LOG.emit("error", msg)
                # Move conn into err state
                self._error_state(context, conn, msg)
        ...
        if not on_delete:
            ...
            for tconn in copy_conns:
                if tconn['status'] == (
                            const.STATE_PENDING and tconn in tenant_conns):
                    tenant_conns.remove(tconn)
        return tenant_conns

    def _sslvpn_push_route(self, context, mgmt_fip, conn):
        svc_context = self.agent.get_sslvpn_contexts(
            context, conn_id=conn['id'])[0]

        route_cidr = self._get_ipsec_tunnel_local_cidr(svc_context)
        route = {'route': route_cidr}

        try:
            RestApi(mgmt_fip).post("ssl-vpn-push-route", route)
        except Exception as err:
            msg = ("SSL: Failed to push routes for SSL VPN. %s"
                   % str(err).capitalize())
            LOG.emit("error", msg)
        self._init_state(context, conn)

    def _get_host(self, uri):
        """ Read host name from auth_uri"""
        expr = re.search(r'(http[s]?)://(.*)(:[\d]+[/]?)', uri, re.I)
        if expr:
            return expr.group(2)

    def _sslvpn_push_config(self, context, mgmt_fip, svc_context):
        LOG.emit("info", "SSL: Pushing auth configuration to %s" % mgmt_fip)
        ...
        remote_vpn_role_name = cfg.CONF.keystone_authtoken.remote_vpn_role_name
        auth_host = self._get_host(auth_uri)
        host_mapping = self._get_hostmapping(auth_host)

        auth_server = {}
        auth_server['auth_uri'] = auth_uri
        auth_server['admin_user'] = admin_user
        auth_server['admin_password'] = admin_password
        auth_server['admin_tenant_name'] = admin_tenant_name
        auth_server['remote_vpn_role_name'] = remote_vpn_role_name
        auth_server['project_id'] = svc_context['service']['tenant_id']
        if host_mapping:
            auth_server['host_mapping'] = host_mapping

        try:
            RestApi(mgmt_fip).post("auth-server-config", auth_server)
        except Exception as err:
            msg = ("SSL: Failed to push SSL VPN config. %s"
                   % str(err).capitalize())
            LOG.emit("error", msg)

    def _get_hostmapping(self, uri):
        LOG.emit("info", "SSL: get host mapping from /etc/hosts, host %s"
                 % uri)
        with open("/etc/hosts", "r") as f:
            mapping = [line for line in f if uri in line]
        if mapping:
            return mapping[0]
        else:
            return uri

    def delete_source_route(self, floating_ip, source_cidrs):
        route_info = []
        for source_cidr in source_cidrs:
            route_info.append({'source_cidr': source_cidr})
        LOG.emit("debug", "SSL: Delete source routes: %s" % route_info)
        RestApi(floating_ip).delete(
            "delete-source-route", None, data=route_info)

    def _sslvpn_create_conn(self, context, mgmt_fip, conn):
        """
        First ssl vpn conn for tenant -
        Also push the configuration
        """
        svc_context = self.agent.get_sslvpn_contexts(
            context, conn_id=conn['id'])[0]

        self._sslvpn_push_config(context, mgmt_fip, svc_context)
        route_cidr = self._get_ipsec_tunnel_local_cidr(svc_context)
        svc_context['service']['cidr'] = route_cidr
        svc_context['sslvpnconns'][0]['connection']['stitching_fixed_ip'] = (
            self._get_stitching_fixed_ip(conn))
        msg = ("SSL: %s, Pushing ssl-vpn configuration." % conn['id'])
        LOG.emit("info", msg)
        RestApi(mgmt_fip).post("create-ssl-vpn-conn", svc_context)
        gateway_ip = self._get_stitching_gw_from_desc(conn)
        msg = ("SSL: %s Adding a default route for stitching interface"
               % conn['id'])
        LOG.emit("info", msg)
        RestApi(mgmt_fip).post("add-stitching-route", {
                                                'gateway_ip': gateway_ip})
        self.delete_source_route(
            mgmt_fip, [route_cidr, self._get_stitching_fixed_ip(conn)])
        self._init_state(context, conn)

    def create_sslvpn_conn(self,  context, mgmt_fip, **kwargs):
        conn = kwargs.get('resource')
        LOG.emit("info", "SSL: create ssl conn %s" % conn)
        try:
            self._sslvpn_create_conn(context, mgmt_fip, conn)
        except exc.ResourceErrorState as rex:
            raise rex
        except Exception as ex:
            msg = "SSL: Exception in create ssl vpn conn %s" % ex
            LOG.emit("error", msg)
            self._error_state(context, conn, msg)

    def _sslvpn_delete_conn(self, context, mgmt_fip, conn):

        RestApi(mgmt_fip).delete(
            "delete-ssl-vpn-conn", {'tunnel': conn['name']})
        gateway_ip = self._get_stitching_gw_from_desc(conn)
        RestApi(mgmt_fip).delete("delete-stitching-route", {
                                                'gateway_ip': gateway_ip})

    def _sslvpn_delete_route(self, context, mgmt_fip, conn):
        route_cidr = self.\
            _get_ipsec_tunnel_local_cidr_from_vpnsvc(conn)

        RestApi(mgmt_fip).delete(
            "delete-ssl-vpn-route", {'route': route_cidr})

    def delete_sslvpn_conn(self,  context, mgmt_fip, **kwargs):
        conn = kwargs.get('resource')
        LOG.emit("info", "SSL: delete ssl conn %s" % conn)
        try:
            self._sslvpn_delete_conn(context, mgmt_fip, conn)
        except exc.ResourceErrorState as rex:
            raise rex
        except Exception as ex:
            msg = "SSL: Exception in delete ssl vpn conn %s" % ex
            LOG.emit("error", msg)
            self._error_state(context, conn, msg)

    def update_sslvpn_conn(self,  context, mgmt_ip, **kwargs):
        pass

    def check_status(self,  context, svc_context):
        conns = svc_context['sslvpnconns']
        for conn in conns:
            conn_status = conn['connection']['status']
            if conn_status == const.STATE_INIT:
                self.agent.update_conn_status(
                    context,
                    const.SERVICE_TYPE_OPENVPN,
                    conn['connection'],
